// SHARED REGRESSION SUITE LOGIC // <--- CHANGE 1: Update comment
// This file contains the common functions for all granular Regression Test Modules.
...
// CRITICAL GLOBAL VARIABLE: This holds the specific key for the current module (e.g., 'testingDataRegressionRouteMaster'). // <--- CHANGE 2: Update comment example
// It is set when the page loads via the initializeRegressionModule function at the bottom.
...
function updateStatusAndTimestamp(selectElement, timestampId) {
    ...
    if (CURRENT_MODULE_KEY) {
        try {
            ...
            if (window.renderRegressionMetrics) { // <--- CHANGE 3: Update Global Metrics Call to Regression
                // Use setTimeout to run this asynchronously to prevent UI lag
                setTimeout(window.renderRegressionMetrics, 0); 
            }
        } catch (error) {
...
async function sendDataToServer() {
    ...
    try {
        ...
        if (window.renderRegressionMetrics) { // <--- CHANGE 4: Update Global Metrics Call to Regression
            await window.renderRegressionMetrics();
        }
 ...
 * @param {string} key - The unique storage key for this module (e.g., 'testingDataRegressionRouteMaster'). // <--- CHANGE 5: Update comment example
 * @param {Array} initialTests - The module-specific static test cases array.
 */
async function initializeRegressionModule(key, initialTests) { // <--- CHANGE 6: Function name changed
...
window.initializeRegressionModule = initializeRegressionModule; // <--- CHANGE 7: Exposed function name changed
...
function addTestCase() {
    ...
    const newCase = {
        ...
        testCase: 'User-Added Regression Case', // <--- CHANGE 8: Update case description
        testSteps: '1. Step 1.\n2. Step 2.\n3. Verify result.',
        testData: 'Input Data',
...
function resetTestCases() {
    ...
    try {
        ...
        if (window.renderRegressionMetrics) { // <--- CHANGE 9: Update Global Metrics Call to Regression
            window.renderRegressionMetrics();
        }
